...
export class AuthService {
  ...
  public signUp$(username: string, email: string, password: string) {
    ...
    return this.http
    .post(`${environment.baseUrl}users`, authModel)
    .pipe(tap((response: any) => this.processTokenResponse(response.id_token)));
  }

  public signIn$(email: string, password: string) {
    const authModel = new AuthModel(email, password);
    return this.http
    .post(`${environment.baseUrl}sessions/create`, authModel)
    .pipe(tap((response: any) => this.processTokenResponse(response.id_token)));
  }
